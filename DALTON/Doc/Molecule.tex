\chapter{Molecule input format}\label{ch:molinp}
...
The molecule input format is originally based on the input for the \mol\
...
the input structure for the \molinp\ file has undergone major changes,
...
options. One option is to use a common basis set for the entire molecule,
...
The fourth line looks a bit devastating. However, for ordinary
...
\item[Basis] \verb|(Character)|. If \verb|ATOMBASIS| has been specified,
  ...
  \begin{description}
  \item{\verb|Basis="basis set name"|} {\it e.g.\/} \verb|Basis=6-31G**|.
  \item{\verb|Basis=INTGRL|} \index{INTGRL}. The basis set is specified in
  the .mol file for this atom type in the same way as if line 1 keyword had been 
  \verb|INTGRL|. The keyword \verb|Blocks| \emph{must} also be specified.
  \item{\verb|Basis=pointcharge|} \index{point charge} The
  atoms in this block will be treated as point charges, that is,
  having only a charge but no basis functions attached to them.
  \end{description}
...
the output.
Two atom names, \verb|Gh| and \verb|IP| \index{IP!atom name in .mol file}, have a special meaning: the associated
...
The second Z-matrix line consists of an atom name, a running number,
the number of the atom to which this atom is bonded with a given bond
length in \angstrom{}, and then finally the nuclear charge of this atom.
Note that the atom label is not used to identify the atom type
and can be used to distinguish atoms, as in the ammonia example above.
...
\section{Adding ghost atoms}
Ghost atoms can be specified with the ATOMBASIS input by adding 
a \verb|Ghost| keyword to the atomtype specification line.
Ghost atoms are useful for example when correcting for 
Basis Set Superposition Errors (BSSE) in intermolecular interactions.
As an example, the following molecule input:
\begin{verbatim}
ATOMBASIS
...
\end{verbatim}
describes one regular water molecule (with nuclei) with the \verb|6-31G*|,
as well as just the \verb|6-31G*| basis functions from another water molecule.

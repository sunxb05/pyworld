...
/* THIS IS A PRELIMINARY IMPLEMENTATION OF THE rCAM-B3LYP FUNCTIONAL OF COHEN, MORI-SANCHEZ,
   AND YANG. (see JCP 126 191109 (2007))
   
   THE IMPLEMENTATION IS A MODIFICATION OF THE USUAL CAM-B3LYP DALTON CODE

   THE ENERGY EXPRESSION IS MODIFIED ACCORDING TO

   E_xc = ALPHA*E_x^{HF} + (1-ALPHA)*E_x^{Slater} + c^{B88}DeltaE_{x}^{B88}      <----- PART 1
                         + BETA*(E_{x}^{LR,HF} - E_{x}^{LR,B88}) + E_{c}^{LYP}   <----- PART 2
   
   WITH CAM PARAMETERS
     
   MU = 0.33, ALPHA = 0.18352, BETA = 0.94979, and c^{B88} = 0.95238

   THE DIFFERENCE COMPARED TO THE STANDARD CAM IMPLEMENTATION IS THAT THE WEIGHT IN FRONT OF
   Ex^{B88} IS DIFFERENT IN PARTS 1 AND 2 */
...
static real CamMuFactor = 0.33, CamAlpha = 0.18352, 
    CamBeta = 0.94979,
    CamLypWeight = 1.00, CamVwnWeight = 0.0;
...
#define COHEN_B88_WEIGHT 0.95238
...
static void rcamb3lyp_second(FunSecondFuncDrv *ds, real factor,
                            const FunDensProp* dp);
static void rcamb3lyp_third(FunThirdFuncDrv *ds, real factor,
                           const FunDensProp* dp);
static void rcamb3lyp_fourth(FunFourthFuncDrv *ds, real factor,
                            const FunDensProp* dp);

Functional rCAM_B3LYPFunctional = {
  "rCamb3lyp",       /* name */
  rcamb3lyp_isgga,   /* gga-corrected */
  1,
  rcamb3lyp_read,
  rcamb3lyp_report,
  rcamb3lyp_energy,
...
rcamb3lyp_read(const char *conf_line)
{
    ...
    if(!parse_table("RCAM-B3LYP", conf_line,
                    ELEMENTS(rcam_keywords), rcam_keywords, weights))
        return 0;
    /* sanity checks */
 ...
 * =================================================================== */
static real
rcamb3lyp_b_energy_small(real a)
{
    real res;
...
static real
rcamb3lyp_b_first_small(real a)
{
    real res;
    a = 2*a; /* the expension derived for different a; correct for this. */
...
static real
rcamb3lyp_b_first_large(real a)
{
    real tmp;
    real ac, a2;
    integer i;
 ...
 * =================================================================== */
static real
rcamb3lyp_b_energy_medium(real a)
{
...
/* tested version */
static real
rcamb3lyp_b_first_medium(real a)
{
    real t1 = 1/a;
    real t2 = a*a;
    real t3 = 1/t2;
...
static real
rcamb3lyp_b_second_medium(real a)
{
...
static real
rcamb3lyp_b_third_medium(real a)
{
...
static real
rcamb3lyp_b_fourth_medium(real a)
{
...
#define EVALUATOR(a,type) \
  ((a<0.14) ? rcamb3lyp_b_ ## type ## _small(a)  : \
  ((a<4.25) ? rcamb3lyp_b_ ## type ## _medium(a) : \
   rcamb3lyp_b_ ## type ## _large(a)))

static real
rcamb3lyp_energy_sigma(real rho, real ex)
{
    real a       = fun_a(rho, ex);
...
static real
rcamb3lyp_energy(const FunDensProp *dp)
{
    real res, ea, eb, ex, ex2;
    FunDensProp dsigma;
...
/*CAMT RE-DEFINE ENERGY FUNC */
    ex2 = 0.5*(SlaterFunctional.func(&dsigma)*(1-ALPHA) +
              BeckeFunctional.func(&dsigma)*COHEN_B88_WEIGHT);

    ea = ex2 - BETA*rcamb3lyp_energy_sigma(dp->rhoa, ex);
/*CAMT END ALPHA*/
	...
	dsigma.gradab = dsigma.grada*dsigma.gradb;
        ex = 0.5*(SlaterFunctional.func(&dsigma) +
                  BeckeFunctional.func(&dsigma)*BECKE88_CORR_WEIGHT);
/*CAMT REDFINE ENERGY FUNC */
        ex2 = 0.5*(SlaterFunctional.func(&dsigma)*(1-ALPHA) +
              BeckeFunctional.func(&dsigma)*COHEN_B88_WEIGHT);

        eb = ex2 - BETA*rcamb3lyp_energy_sigma(dp->rhob, ex);
/*CAMT END BETA */
    } else eb = ea;
    res = ea + eb;
#if ADD_CORRELATION
...
static void
rcamb3lyp_first_sigma(real rho, real ex, 
...
static void
rcamb3lyp_first(FunFirstFuncDrv *ds, real factor, const FunDensProp *dp)
{
    RGFirstDrv res, dfun;
    FunDensProp dsigma;
    real ex, ex_weight;
...
/*CAMT FIRST DERIV OF PART 2*/
    ...
    rcamb3lyp_first_sigma(dp->rhoa, ex, &dfun, &res);
/*CAMT FIRST DERIV OF PART 1*/
    memset(&fun1, 0, sizeof(fun1));
    SlaterFunctional.first(&fun1, 1-ALPHA, dp);
    BeckeFunctional.first(&fun1, COHEN_B88_WEIGHT, dp);
    dfun.df10 = fun1.df1000; dfun.df01 = fun1.df0010;
/*CAMT END ALPHA */
    ds->df1000 += factor*(dfun.df10 + res.df10);
    ds->df0010 += factor*(dfun.df01 + res.df01);
...
/*CAMT FIRST DERIV OF PART 2*/
            ...
            rcamb3lyp_first_sigma(dp->rhob, ex, &dfun, &res);
/*CAMT FIRST DERIV OF PART 1*/
            memset(&fun1, 0, sizeof(fun1));
            SlaterFunctional.first(&fun1, 1-ALPHA, dp);
            BeckeFunctional.first(&fun1, COHEN_B88_WEIGHT, dp);
...
/*CAMT END BETA*/
        ...
        ds->df0100 += factor*(dfun.df10 + res.df10);
        ds->df0001 += factor*(dfun.df01 + res.df01);
    }
#if ADD_CORRELATION
...
static void
rcamb3lyp_second_sigma(real rho, real ex, RGSecondDrv *f2,
                      RGSecondDrv *res)
{
    ...
    b_second = BETA*rcamb3lyp_b_second_medium(a);
...
static void
rcamb3lyp_second(FunSecondFuncDrv *ds, real factor, const FunDensProp* dp)
{
    FunSecondFuncDrv f2;
...
/*CAMT SECOND DERIV OF PART 2*/
    ...
    dfun.df11 = f2.df1010; 
    rcamb3lyp_second_sigma(dp->rhoa, ex, &dfun, &res);

/*CAMT SECOND DERIV OF PART 1*/
    memset(&f2, 0, sizeof(f2));
    SlaterFunctional.second(&f2, 1-ALPHA, dp);
    BeckeFunctional.second(&f2, COHEN_B88_WEIGHT, dp);
    ...
    ds->df1000 += factor*(dfun.df10 + res.df10);
    ds->df0010 += factor*(dfun.df01 + res.df01);
    ds->df2000 += factor*(dfun.df20 + res.df20);
    ds->df1010 += factor*(dfun.df11 + res.df11);
    ds->df0020 += factor*(dfun.df02 + res.df02);
/*CAMT END ALPHA */
    if(fabs(dp->rhoa-dp->rhob)>1e-40 || fabs(dp->grada-dp->gradb)>1e-40) {
        dsigma.rhoa  = dsigma.rhob  = dp->rhob;
        dsigma.grada = dsigma.gradb = dp->gradb;
...
/*CAMT SECOND DERIV OF PART 2*/
        ...
        dfun.df11 = f2.df0101;
        rcamb3lyp_second_sigma(dp->rhob, ex, &dfun, &res);
/*CAMT SECOND DERIV OF PART 1*/
        memset(&f2, 0, sizeof(f2));
        SlaterFunctional.second(&f2, 1-ALPHA, dp);
        BeckeFunctional.second(&f2, COHEN_B88_WEIGHT, dp);
...
/* END BETA*/
    ds->df0100 += factor*(dfun.df10 + res.df10);
    ds->df0001 += factor*(dfun.df01 + res.df01);
    ds->df0200 += factor*(dfun.df20 + res.df20);
    ds->df0101 += factor*(dfun.df11 + res.df11);
    ds->df0002 += factor*(dfun.df02 + res.df02);
...
static void
rcamb3lyp_third_sigma(real rho, real ex, RGThirdDrv *f3,
                     RGThirdDrv *res)
{
    ...
    b_first  = BETA*EVALUATOR(a, first);
    b_second = BETA*rcamb3lyp_b_second_medium(a);
    b_third  = BETA*rcamb3lyp_b_third_medium(a);
...
static void
rcamb3lyp_third(FunThirdFuncDrv *ds, real factor, const FunDensProp* dp)
{
    FunThirdFuncDrv f3;
    RGThirdDrv res, dfun;
    FunDensProp dsigma;
...
/*CAMT THIRD DERIV OF PART 2*/
    ...
    rcamb3lyp_third_sigma(dp->rhoa, ex, &dfun, &res);
/*CAMT THIRD DERIV OF PART 1*/
    memset(&f3, 0, sizeof(f3));
    SlaterFunctional.third(&f3, 1-ALPHA, dp);
    BeckeFunctional.third(&f3, COHEN_B88_WEIGHT, dp);
...
/*END ALPHA*/
    ...
    ds->df3000 += factor*(dfun.df30 + res.df30);
    ds->df2010 += factor*(dfun.df21 + res.df21);
    ds->df1020 += factor*(dfun.df12 + res.df12);
    ds->df0030 += factor*(dfun.df03 + res.df03);
...
/* THIRD DERIV OF PART 2*/
        ...
        rcamb3lyp_third_sigma(dp->rhob, ex, &dfun, &res);
/* THIRD DERIV OF PART 3*/
        memset(&f3, 0, sizeof(f3));
        SlaterFunctional.third(&f3, 1-ALPHA, dp);
        BeckeFunctional.third(&f3, COHEN_B88_WEIGHT, dp);
        dfun.df10 = f3.df0100; dfun.df20 = f3.df0200; dfun.df30 = f3.df0300;
...
/* END BETA */
    ...
    ds->df0300 += factor*(dfun.df30 + res.df30);
    ds->df0201 += factor*(dfun.df21 + res.df21);
    ds->df0102 += factor*(dfun.df12 + res.df12);
    ds->df0003 += factor*(dfun.df03 + res.df03);
...
/* ===================================================================
   ...
   =================================================================== */
static void
rcamb3lyp_fourth_sigma(real rho, real ex, RGFourthDrv *f4,
		      RGFourthDrv *res)
{
    ...
    b_second = BETA*rcamb3lyp_b_second_medium(a_);
    b_third  = BETA*rcamb3lyp_b_third_medium(a_);
    b_fourth = BETA*rcamb3lyp_b_fourth_medium(a_);
...
static void
rcamb3lyp_fourth(FunFourthFuncDrv *ds, real factor,
		const FunDensProp* dp)
{
    FunFourthFuncDrv f4;
...
/* FOURTH DERIV OF PART 2*/
    ...
    rcamb3lyp_fourth_sigma(dp->rhoa, ex, &dfun, &res);
/* FOURTH DERIV OF PART 1*/
    memset(&f4, 0, sizeof(f4));
    SlaterFunctional.fourth(&f4, 1-ALPHA, dp);
    BeckeFunctional.fourth(&f4, COHEN_B88_WEIGHT, dp);
...
/* END ALPHA*/
    ...
    ds->df4000 += factor*(dfun.df40 + res.df40);
    ds->df3010 += factor*(dfun.df31 + res.df31);
    ds->df2020 += factor*(dfun.df22 + res.df22);
    ds->df1030 += factor*(dfun.df13 + res.df13);
    ds->df0040 += factor*(dfun.df04 + res.df04);
...
/* FOURTH DERIV OF PART 2*/
	...
	dfun.df13 = f4.df0103; dfun.df04 = f4.df0004;
        rcamb3lyp_fourth_sigma(dp->rhob, ex, &dfun, &res);
/* FOURTH DERIV OF PART 1*/
        memset(&f4, 0, sizeof(f4));
        SlaterFunctional.fourth(&f4, 1-ALPHA, dp);
        BeckeFunctional.fourth(&f4, COHEN_B88_WEIGHT, dp);
...
/* END BETA */
    ...
    ds->df0400 += factor*(dfun.df40 + res.df40);
    ds->df0301 += factor*(dfun.df31 + res.df31);
    ds->df0202 += factor*(dfun.df22 + res.df22);
    ds->df0103 += factor*(dfun.df13 + res.df13);
    ds->df0004 += factor*(dfun.df04 + res.df04);
...
integer main(integer argc, char *argv[])
{
    ...
    for(a=0.1; a<10; a += 0.1) {
        bfactor_first  = rcamb3lyp_b_second_medium(a*FAC);//EVALUATOR(a, second);

...
!!
!!  This file calculates the MCP Version 1 potential integrals using
...
#include "stdout.h"
  ...
  !> \brief calculates the MCP Version 1 potential integrals using
  !>        contracted Cartesian Gaussians
  !> \author Bin Gao
  !> \date 2011-09-24
  ...
  subroutine contr_cgto_mcp1_pot(idx_bra, coord_bra, angular_bra, num_prim_bra, &
...
!f2py depend(num_opt) :: contr_ints
    ...
    if (num_cents==0) then
      call contr_cgto_mcp1_pot_recurr(coord_bra, angular_bra, num_prim_bra,        &
                                    exponent_bra, num_contr_bra, contr_coef_bra, &
      ...
      ! total geometric derivatives of MCP Version 1 potential integrals
      if (order_mom==0) then
        ...
        ! we need to scatter the partial and total geometric derivatives into appropriate places
          ...
          if (ierr/=0) stop "contr_cgto_mcp1_pot>> failed to allocate part_cints/1!"
      ...
      ! total geometric derivatives of MCP Version 1 potential with Cartesian
      ! multipole moment integrals
      else
        stop "contr_cgto_mcp1_pot>> not implemented! please write to authors!"
    ...
    call xtimer_view(curr_time, "contr_cgto_mcp1_pot", STDOUT)
#endif
    return
  end subroutine contr_cgto_mcp1_pot

  !> \brief recurrence relations of MCP Version 1 potential integrals using
  !>        contracted Cartesian Gaussians
  !> \author Bin Gao
  !> \date 2011-09-24
  !> \param coord_bra contains the coordinates of bra center
  ...
  !> \param num_opt is the number of operators including derivatives
  !> \return contr_ints contains the contracted Cartesian multipole moment integrals
  subroutine contr_cgto_mcp1_pot_recurr(coord_bra, angular_bra, num_prim_bra,        &
                                      exponent_bra, num_contr_bra, contr_coef_bra, &
                                      coord_ket, angular_ket, num_prim_ket,        &
                                      exponent_ket, num_contr_ket, contr_coef_ket, &
                                      order_geo_bra, order_geo_ket,                &
...
#if defined(XTIME)
    ! prints the CPU elapsed time
    call xtimer_view(curr_time, "contr_cgto_mcp1_pot_recurr", STDOUT)
#endif
    return
100 format("contr_cgto_mcp1_pot_recurr>> ",A,I8,1X,A)
  end subroutine contr_cgto_mcp1_pot_recurr

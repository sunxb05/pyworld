...
!!  * first version
  ...
  !> \brief tests subroutines in contr_csgto_carmom.F90
  !> \author Bin Gao
  !> \date 2011-10-07
  !> \param io_log is the IO unit of log file
  !> \return test_failed indicates if the test is failed
  subroutine test_contr_csgto_carmom(io_log, test_failed)
    use xkind
...
#include "water_ccpv6z_sgto_carmom.h"
    ...
    logical different                   !if result from \fn(contr_csgto_carmom) is different from reference
    ...
    do itst = 1, NUM_TESTS
      ...
      call contr_csgto_carmom(idx_bra, COORDS(3*idx_bra-2:3*idx_bra),        &
                              ...
                              ORDER_CENT(jtst:ltst), num_sgto_bra,           &
                              num_sgto_ket, num_opt, contr_ints)
      ! gets the end time
      call xtimer_set(end_time)
      ! prints the CPU elapsed time
      call html_log_real_number(log_text="Time (s) used for contr_csgto_carmom:", &
            ...
            do icontr = 1, NUM_CONTR_BLOCK(iblock)
              do isgto = 1, num_sgto_bra
                ...
                if (different) then
                  ...
                  call html_log_real_number(                                  &
                         log_text="Result from contr_csgto_carmom:",          &
    ...
    return
  end subroutine test_contr_csgto_carmom

...
!!
!!  Tests subroutine prim_hgto_carmom.
!!
!!  2011-07-27, Bin Gao
!!  * first version

  !> \brief tests subroutine prim_hgto_carmom
  !> \author Bin Gao
  !> \date 2011-07-27
  !> \param io_log is the IO unit of log file
  !> \return test_failed indicates if the test is failed
  subroutine test_prim_hgto_carmom(io_log, test_failed)
    use xkind
...
#include "test_f90/prim_gto.h"
    ...
    ! the following tests are for individual cases in \fn(prim_hgto_carmom)
    orders_hgto_bra = (/0,0/)
    orders_hgto_ket = (/0,0/)
    order_mom = 3
    order_elec = 0
    call sub_test_prim_hgto_carmom(orders_hgto_bra, orders_hgto_ket, order_mom, &
    ...
    call sub_test_prim_hgto_carmom(orders_hgto_bra, orders_hgto_ket, order_mom, &
                                   order_elec, io_log, test_failed)
    return
  end subroutine test_prim_hgto_carmom

  !> \brief tests subroutine prim_hgto_carmom with specific orders of
  ...
  !> \param io_log is the IO unit of log file
  !> \return test_failed indicates if the test is failed
  subroutine sub_test_prim_hgto_carmom(orders_hgto_bra, orders_hgto_ket, order_mom, &
                                       order_elec, io_log, test_failed)
    use xkind
    ! module of HTML test log routines
    use html_log
...
#include "test_f90/prim_gto.h"
    ...
    real(REALK), allocatable :: hgto_pints(:,:,:,:)
                                    !integrals from \fn(prim_hgto_carmom)
    ...
    integer addr_hbra, addr_hket, addr_elec, addr_mom
                                    !addresses of \var(hgto_pints)
    integer ierr                    !error information
    real(REALK) begin_time          !begin of CPU time
    real(REALK) end_time            !end of CPU time
    logical different               !if result from \fn(prim_hgto_carmom) is different from reference
    ! dumps information of tests
    ...
    ! allocates memory for the integrals from \fn(prim_hgto_carmom)
    ...
    allocate(hgto_pints(dim_hgto_bra,dim_hgto_ket,num_elec,num_mom), stat=ierr)
    if (ierr/=0) then
      call html_log_int_number(log_text="Failed to allocate hgto_pints:",          &
                               log_int=dim_hgto_bra*dim_hgto_ket*num_elec*num_mom, &
                               fmt_int="I12", io_log=io_log, font_color="red")
      test_failed = .true.
      return
    end if
    call prim_hgto_carmom(orders_hgto_bra, COORD_BRA, EXPONENT_BRA,      &
                          orders_hgto_ket, COORD_KET, EXPONENT_KET,      &
                          order_elec, DIPORG, SCAL_CONST, order_mom,     &
                          dim_hgto_bra, dim_hgto_ket, num_elec, num_mom, &
                          hgto_pints)
    ! gets the end time
    call xtimer_set(end_time)
    ! prints the CPU elapsed time
    call html_log_real_number(log_text="Time (s) used for prim_hgto_carmom:", &
    ...
    ! initializes the address of integrals from \fn(prim_hgto_carmom)
    addr_mom = 0
                    ...
                    do z_hbra = 0, order_hbra
                      do y_hbra = 0, order_hbra-z_hbra
                        ...
                        call check_difference(recur_pint,                          &
                               hgto_pints(addr_hbra,addr_hket,addr_elec,addr_mom), &
                               different)
                        if (different) then
                          ...
                          call html_log_real_number(                             &
                                 ...
                                 io_log=io_log, font_color="blue")
                          call html_log_real_number(                                &
                                 log_text="Result from prim_hgto_carmom:",          &
    ...
    return
  end subroutine sub_test_prim_hgto_carmom

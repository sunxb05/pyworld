...
!!
!!  Tests subroutine prim_hgto_delta.
!!
!!  2012-03-19, Bin Gao
!!  * first version

  !> \brief tests subroutine prim_hgto_delta
  !> \author Bin Gao
  !> \date 2012-03-19
  !> \param io_log is the IO unit of log file
  !> \return test_failed indicates if the test is failed
  subroutine test_prim_hgto_delta(io_log, test_failed)
    use xkind
...
#include "test_f90/prim_gto.h"
    ...
    integer order_geo_pot       !order of geometric derivatives on Dirac delta function origin
    ...
    call html_log_real_array(log_text="Coordinates of Dirac delta function origin:", &
                             log_real=DELORG, fmt_real="F16.8", io_log=io_log)
    ...
    ! the following tests are for individual cases in \fn(prim_hgto_delta)
    orders_hgto_bra = (/0,0/)
    orders_hgto_ket = (/0,0/)
    order_mom = 3
    order_geo_pot = 3
    order_elec = 0
    call sub_test_prim_hgto_delta(orders_hgto_bra, orders_hgto_ket, order_mom, &
    ...
    call sub_test_prim_hgto_delta(orders_hgto_bra, orders_hgto_ket, order_mom, &
                                  order_elec, order_geo_pot, io_log, test_failed)
    return
  end subroutine test_prim_hgto_delta

  !> \brief tests subroutine prim_hgto_delta with specific orders of
  ...
  !> \param order_geo_pot is the order of geometric derivatives on Dirac delta function origin
  !> \param order_elec is the order of electronic derivatives
  !> \param io_log is the IO unit of log file
  !> \return test_failed indicates if the test is failed
  subroutine sub_test_prim_hgto_delta(orders_hgto_bra, orders_hgto_ket, order_mom, &
                                      order_elec, order_geo_pot, io_log, test_failed)
    use xkind
    ! module of HTML test log routines
    use html_log
...
#include "test_f90/prim_gto.h"
    ...
    integer num_geo_pot             !number of geometric derivatives on Dirac delta function origin
    real(REALK), allocatable :: hgto_pints(:,:,:,:,:)
                                    !integrals from \fn(prim_hgto_delta)
    ...
    logical different               !if result from \fn(prim_hgto_delta) is different from reference
    ! dumps information of tests
    ...
    call html_log_int_number(                                               &
           "Order of geometric derivative on Dirac delta function origin:", &
           order_geo_pot, "I3", io_log)
    ! gets the begin time
    call xtimer_set(begin_time)
    ! allocates memory for the integrals from \fn(prim_hgto_delta)
    ...
    call prim_hgto_delta(orders_hgto_bra, COORD_BRA, EXPONENT_BRA, &
                         orders_hgto_ket, COORD_KET, EXPONENT_KET, &
                         order_elec, DELORG, DIPORG, SCAL_CONST,   &
    ...
    call html_log_real_number(log_text="Time (s) used for prim_hgto_delta:",  &
    ...
    ! initializes the address of integrals from \fn(prim_hgto_delta)
    addr_geo = 0
                      ...
                      do order_hbra = orders_hgto_bra(1), orders_hgto_bra(2)
                        do z_hbra = 0, order_hbra
                          do y_hbra = 0, order_hbra-z_hbra
                            ...
                            recur_pint = recur_delta_moment((/x_hbra,y_hbra,z_hbra/), &
                                                            (/x_hket+x_elec,          &
                                                              y_hket+y_elec,          &
                                                              z_hket+z_elec/),        &
                                                            (/x_mom,y_mom,z_mom/),    &
                            ...
                            if (different) then
                              ...
                              call html_log_real_number(                            &
                                     log_text="Reference from recur_delta_moment:", &
                                     log_real=recur_pint, fmt_real="Es20.12",       &
                                     io_log=io_log, font_color="blue")
                              call html_log_real_number(                                &
                                     log_text="Result from prim_hgto_delta:",           &
    ...
    return
  end subroutine sub_test_prim_hgto_delta

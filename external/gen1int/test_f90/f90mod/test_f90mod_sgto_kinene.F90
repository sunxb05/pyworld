...
!!  * first version
  ...
  subroutine test_f90mod_sgto_kinene(io_log, test_failed)
    use xkind
...
#include "test_f90/water_ccpv6z_sgto_kinene.h"
    ...
    test_failed = .false.
    call html_log_text("Tests kinetic energy integrals of water molecule "// &
                       "using cc-pV6Z spherical GTOs ...", io_log)
    ! initializes the information of kinetic energy integrals
    call OnePropCreate(prop_name=INT_KIN_ENERGY, one_prop=one_prop, &
                       info_prop=ierr)
    ...
    return
  end subroutine test_f90mod_sgto_kinene

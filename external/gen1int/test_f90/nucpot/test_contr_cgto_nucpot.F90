...
!!  * first version
  ...
  !> \brief tests subroutines in contr_cgto_nucpot.F90
  !> \author Bin Gao
  !> \date 2011-09-29
  !> \param io_log is the IO unit of log file
  !> \return test_failed indicates if the test is failed
  subroutine test_contr_cgto_nucpot(io_log, test_failed)
    use xkind
...
#include "water_ccpv6z_cgto_nucpot.h"
    ...
    logical different                   !if result from \fn(contr_cgto_nucpot) is different from reference
    ...
    do itst = 1, NUM_TESTS
      ...
      call contr_cgto_nucpot(idx_bra, COORDS(3*idx_bra-2:3*idx_bra),        &
      ...
      call html_log_real_number(log_text="Time (s) used for contr_cgto_nucpot:", &
            ...
            do icontr = 1, NUM_CONTR_BLOCK(iblock)
              do icgto = 1, num_cgto_bra
                ...
                if (different) then
                  ...
                  call html_log_real_number(                                &
                         log_text="Result from contr_cgto_nucpot:",         &
    ...
    return
  end subroutine test_contr_cgto_nucpot

...
!!  * first version
  ...
  !> \brief tests subroutines in contr_sgto_nucpot.F90
  !> \author Bin Gao
  !> \date 2011-09-30
  !> \param io_log is the IO unit of log file
  !> \return test_failed indicates if the test is failed
  subroutine test_contr_sgto_nucpot(io_log, test_failed)
    use xkind
...
#include "water_ccpv6z_sgto_nucpot.h"
    ...
    logical different                   !if result from \fn(contr_sgto_nucpot) is different from reference
    ...
    do itst = 1, NUM_TESTS
      ...
      if (IDX_NUCORG(itst)>0) &
        call html_log_int_number("Index of nuclear potential origin:", &
                                 IDX_NUCORG(itst), "I4", io_log)
      call html_log_real_array("Coordinates of nuclear potential origin:", &
                               NUC_ORIGIN(jtst:ktst), "F16.8", io_log)
      ...
      call html_log_int_number(                                             &
             "Order of geometric derivatives on nuclear potential origin:", &
      ...
      call contr_sgto_nucpot(idx_bra, COORDS(3*idx_bra-2:3*idx_bra),        &
                             ...
                             ORDER_ELEC(itst), IDX_NUCORG(itst),            &
                             NUC_ORIGIN(jtst:ktst), IDX_DIPORG(itst),       &
      ...
      call html_log_real_number(log_text="Time (s) used for contr_sgto_nucpot:", &
            ...
            do icontr = 1, NUM_CONTR_BLOCK(iblock)
              do isgto = 1, num_sgto_bra
                ...
                if (different) then
                  ...
                  call html_log_real_number(                                  &
                         log_text="Result from contr_sgto_nucpot:",           &
    ...
    return
  end subroutine test_contr_sgto_nucpot

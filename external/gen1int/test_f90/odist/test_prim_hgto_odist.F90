...
#include "test_f90/prim_gto.h"
    ...
    ! the following tests are for individual cases in \fn(prim_hgto_odist)
    orders_hgto_bra = (/0,0/)
    orders_hgto_ket = (/0,0/)
    call sub_test_prim_hgto_odist(orders_hgto_bra, orders_hgto_ket, io_log, test_failed)
    orders_hgto_bra = (/0,1/)
    orders_hgto_ket = (/0,0/)
    call sub_test_prim_hgto_odist(orders_hgto_bra, orders_hgto_ket, io_log, test_failed)
    orders_hgto_bra = (/0,0/)
    ...
    call sub_test_prim_hgto_odist(orders_hgto_bra, orders_hgto_ket, io_log, test_failed)
    return
  end subroutine test_prim_hgto_odist

  !> \brief tests subroutine prim_hgto_odist with specific orders of HGTOs
  !> \author Bin Gao
  !> \date 2012-02-10
  ...
  !> \param io_log is the IO unit of log file
  !> \return test_failed indicates if the test is failed
  subroutine sub_test_prim_hgto_odist(orders_hgto_bra, orders_hgto_ket, io_log, test_failed)
    use xkind
    ! module of HTML test log routines
    use html_log
...
#include "test_f90/prim_gto.h"
    ...
    integer dim_hgto_ket                           !number of Hermite Gaussians on ket center
    real(REALK), allocatable :: hgto_odist(:,:,:)  !overlap distribution using primitive Hermite Gaussians
    ...
    real(REALK) elec_coord_ket(3)   !electron coordinates on ket center
    real(REALK) recur_hbra          !value of HGTO on bra center from \fn(recur_hgto_value)
    real(REALK) recur_hket          !value of HGTO on ket center from \fn(recur_hgto_value)
    real(REALK) recur_odist         !value of HGTO overlap distribution from \fn(recur_hgto_value)
    integer ipoint                  !incremental recorder over grid points
    integer addr_hbra, addr_hket    !addresses of \var(hgto_odist)
    integer ierr                    !error information
    real(REALK) begin_time          !begin of CPU time
    real(REALK) end_time            !end of CPU time
    logical different               !if result from \fn(prim_hgto_odist) is different from reference
    ! dumps information of tests
    ...
    ! allocates memory for \fn(prim_hgto_odist)
    ...
    if (ierr/=0) then
      call html_log_int_number(log_text="Failed to allocate hgto_odist:",    &
                               log_int=dim_hgto_bra*dim_hgto_ket*num_points, &
                               fmt_int="I12", io_log=io_log, font_color="red")
      test_failed = .true.
      deallocate(grid_points)
      return
    end if
    call prim_hgto_odist(orders_hgto_bra, COORD_BRA, EXPONENT_BRA, &
    ...
    call html_log_real_number(log_text="Time (s) used for prim_hgto_odist:",  &
    ...
    do ipoint = 1, num_points
      ...
      elec_coord_ket = grid_points(:,ipoint)-COORD_KET
      ! initializes the address of overlap distribution from \fn(prim_hgto_odist)
      addr_hket = 0
      ...
      do order_hket = orders_hgto_ket(1), orders_hgto_ket(2)
        do z_hket = 0, order_hket
          do y_hket = 0, order_hket-z_hket
            ...
            ! value of HGTO on ket center from \fn(recur_hgto_value)
            recur_hket = recur_hgto_value((/x_hket,y_hket,z_hket/), &
                                          -0.5_REALK/EXPONENT_KET,  &
                                          elec_coord_ket)
            ! initializes the address of overlap distribution from \fn(prim_hgto_odist)
            addr_hbra = 0
            ...
            do order_hbra = orders_hgto_bra(1), orders_hgto_bra(2)
              do z_hbra = 0, order_hbra
                do y_hbra = 0, order_hbra-z_hbra
                  ...
                  recur_hbra = recur_hgto_value((/x_hbra,y_hbra,z_hbra/), &
                                                -0.5_REALK/EXPONENT_BRA,  &
                                                elec_coord_bra)
                  ! HGTO overlap distribution from \fn(recur_hgto)
                  recur_odist = recur_hbra*recur_hket
                  call check_difference(recur_odist,                            &
                                        hgto_odist(addr_hbra,addr_hket,ipoint), &
                                        different)
                  if (different) then
                    ...
                    call html_log_real_number(                          &
                           ...
                           log_real=recur_odist, fmt_real="Es20.12",    &
                           io_log=io_log, font_color="blue")
                    call html_log_real_number(                              &
                           log_text="Result from prim_hgto_odist:",         &
                           log_real=hgto_odist(addr_hbra,addr_hket,ipoint), &
    ...
    return
  end subroutine sub_test_prim_hgto_odist

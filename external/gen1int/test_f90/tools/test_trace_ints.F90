...
!!
!!  Tests subroutines in trace_ints.F90.
!!
!!  2011-08-04, Bin Gao
!!  * first version

  !> \brief tests subroutines in trace_ints.F90
  !> \author Bin Gao
  !> \date 2011-08-04
  !> \param io_log is the IO unit of log file
  !> \return test_failed indicates if the test is failed
  subroutine test_trace_ints(io_log, test_failed)
    use xkind
    ...
    real(REALK) ro_sints(NUM_SGTO_BRA,NUM_CONTR_BRA, &    !reordered SGTO integrals
                         NUM_SGTO_KET,NUM_CONTR_KET,NUM_OPT)
    real(REALK) dens_sints(NUM_SGTO_KET,NUM_CONTR_KET, &  !atomic-orbital density matrix using SGTOs
                           NUM_SGTO_BRA,NUM_CONTR_BRA,NUM_OPT)
    ...
    real(REALK) ro_cints(NUM_CGTO_BRA,NUM_CONTR_BRA, &    !reordered CGTO integrals
                         NUM_CGTO_KET,NUM_CONTR_KET,NUM_OPT)
    real(REALK) dens_cints(NUM_CGTO_KET,NUM_CONTR_KET, &  !atomic-orbital density matrix using CGTOs
                           NUM_CGTO_BRA,NUM_CONTR_BRA,NUM_OPT)
    real(REALK) tr_ints(NUM_OPT)                          !traces of integrals
    real(REALK) ref_tr(NUM_OPT)                           !referenced traces of integrals
          ...
          do icontr = 1, NUM_CONTR_BRA
            do ibra = 1, NUM_SGTO_BRA
              sgto_ints(ibra,icontr,iket,jcontr,iopt)                            &
                = real(ibra,REALK)*0.01_REALK+real(icontr,REALK)*0.001_REALK     &
                + real(iket,REALK)*0.0001_REALK+real(jcontr,REALK)*0.00001_REALK &
                + real(iopt,REALK)*0.000001_REALK
            end do
          end do
        end do
      end do
    end do
    ! sets the atomic-orbital density matrix
    do iopt = 1, NUM_OPT
      do icontr = 1, NUM_CONTR_BRA
        do ibra = 1, NUM_SGTO_BRA
          do jcontr = 1, NUM_CONTR_KET
            do iket = 1, NUM_SGTO_KET
              dens_sints(iket,jcontr,ibra,icontr,iopt)                           &
                = real(ibra,REALK)*0.01_REALK+real(icontr,REALK)*0.001_REALK     &
                + real(iket,REALK)*0.0001_REALK+real(jcontr,REALK)*0.00001_REALK &
                + real(iopt,REALK)*0.000001_REALK
            end do
    ...
    ! gets the traces from \fn(trace_gto_ints) as referenced results
    ref_tr = 0.0_REALK
    call trace_gto_ints(NUM_SGTO_BRA, NUM_CONTR_BRA, NUM_SGTO_KET, NUM_CONTR_KET, &
                        NUM_OPT, ro_sints, dens_sints, ref_tr)
    ! dumps information of tests
    call html_log_heading(heading="Tests subroutine trace_sgto_ints", &
                          io_log=io_log, level=4)
    ! gets the begin time
    call xtimer_set(begin_time)
    ! gets the traces using \fn(trace_sgto_ints)
    tr_ints = 0.0_REALK
    call trace_sgto_ints(ANG_BRA, NUM_SGTO_BRA, mag_bra, &
                         ANG_KET, NUM_SGTO_KET, mag_ket, &
                         NUM_CONTR_BRA, NUM_CONTR_KET,   &
                         NUM_OPT, sgto_ints, dens_sints, tr_ints)
    ! gets the end time
    call xtimer_set(end_time)
    ! prints the CPU elapsed time
    call html_log_real_number(log_text="Time (s) used for trace_sgto_ints:",  &
                              log_real=end_time-begin_time, fmt_real="F10.4", &
                              io_log=io_log, font_color="blue")
    ! checks the results
    do iopt = 1, NUM_OPT
      call check_difference(ref_tr(iopt), tr_ints(iopt), different)
      if (different) then
        call html_log_int_number("ID of operator:", iopt, "I3", io_log)
        call html_log_real_number(log_text="Referenced result:",           &
                                  log_real=ref_tr(iopt), fmt_real="F16.6", &
                                  io_log=io_log, font_color="blue")
        call html_log_real_number(log_text="Result from trace_sgto_ints:",  &
                                  log_real=tr_ints(iopt), fmt_real="F16.6", &
                                  io_log=io_log, font_color="red")
        test_failed = .true.
      end if
    end do
    ! sets CGTOs on bra and centers, which are in the reverse order of Gen1Int
    addr_bra = 0
    do zpow = ANG_BRA, 0, -1
    ...
    do iopt = 1, NUM_OPT
      do icontr = 1, NUM_CONTR_BRA
        do ibra = 1, NUM_CGTO_BRA
          do jcontr = 1, NUM_CONTR_KET
            do iket = 1, NUM_CGTO_KET
              dens_cints(iket,jcontr,ibra,icontr,iopt)                           &
                = real(ibra,REALK)*0.01_REALK+real(icontr,REALK)*0.001_REALK     &
                + real(iket,REALK)*0.0001_REALK+real(jcontr,REALK)*0.00001_REALK &
                + real(iopt,REALK)*0.000001_REALK
            end do
    ...
    ! gets the traces from \fn(trace_gto_ints) as referenced results
    ref_tr = 0.0_REALK
    call trace_gto_ints(NUM_CGTO_BRA, NUM_CONTR_BRA, NUM_CGTO_KET, NUM_CONTR_KET, &
                        NUM_OPT, ro_cints, dens_cints, ref_tr)
    ! dumps information of tests
    call html_log_heading(heading="Tests subroutine trace_cgto_ints", &
                          io_log=io_log, level=4)
    ! resets the begin time
    call xtimer_set(begin_time)
    ! gets the traces using \fn(trace_cgto_ints)
    tr_ints = 0.0_REALK
    call trace_cgto_ints(ANG_BRA, NUM_CGTO_BRA, power_bra, &
                         ANG_KET, NUM_CGTO_KET, power_ket, &
                         NUM_CONTR_BRA, NUM_CONTR_KET,     &
                         NUM_OPT, cgto_ints, dens_cints, tr_ints)
    ! gets the end time
    call xtimer_set(end_time)
    ! prints the CPU elapsed time
    call html_log_real_number(log_text="Time (s) used for trace_cgto_ints:",  &
                              log_real=end_time-begin_time, fmt_real="F10.4", &
                              io_log=io_log, font_color="blue")
    ! checks the results
    do iopt = 1, NUM_OPT
      ...
      if (different) then
        ...
        call html_log_real_number(log_text="Referenced result:",           &
                                  log_real=ref_tr(iopt), fmt_real="F16.6", &
                                  io_log=io_log, font_color="blue")
        call html_log_real_number(log_text="Result from trace_cgto_ints:",  &
                                  log_real=tr_ints(iopt), fmt_real="F16.6", &
                                  io_log=io_log, font_color="red")
        test_failed = .true.
      end if
    ...
    return
  end subroutine test_trace_ints
